...
/*
    Returns a random connected room from the current room
        in: currentRoom - the current room of the ghost or hunter
    return: a randomly selected connected room, or NULL if there are no connected rooms
*/
RoomType *getRandomConnectedRoom(RoomType *currentRoom){
    if (currentRoom == NULL || currentRoom->connectedRooms == NULL)
    {
        return NULL; // No connected rooms available
    }

    // Count the number of connected rooms
    int count = 0;
    RoomNode *node = currentRoom->connectedRooms;
    while (node != NULL)
    {
        count++;
        node = node->next;
    ...
    if (count == 0)
        return NULL; // No connected rooms

    // Select a random room
    int roomIndex = randInt(0, count);
    node = currentRoom->connectedRooms;
